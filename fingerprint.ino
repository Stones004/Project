...
Ticker modeTimer;
Ticker addIDTimer;
Ticker deleteIDTimer;
Ticker t1; // Declare t1 as a Ticker object
Ticker t2; // Declare t2 as a Ticker object

HardwareSerial mySerial(2);
Adafruit_Fingerprint finger = Adafruit_Fingerprint(&mySerial);

// WiFi Credentials
const char *ssid = "mi";          // Replace with your WiFi SSID
const char *password = "123456781";  // Replace with your WiFi password
const char* device_token = "57c386c2";  // Replace with your device token
String URL = "http://192.168.67.206/biometricattendancev2/getdata.php";  // Replace with your server URL
...
void setup() {
  ...
  // Set up timers
  modeTimer.attach_ms(25000, CheckMode);
  CheckMode();  // Initial mode check
}
...
void loop() {
  ...
  if(!WiFi.isConnected() && (millis() - previousMillis >= 10000)) {
  ...
  if (WiFi.isConnected()) {
    ...
    if (payload.substring(0, 4) == "mode") {
      ...
      if (device_Mode && devMode) {
        device_Mode = false;
        t1.detach(); // Stop t1 timer
        t2.detach(); // Stop t2 timer
        Serial.println("Device Mode: Attendance");
      } else if (!device_Mode && !devMode) {
        device_Mode = true;
        t1.attach(10, ChecktoAddID); // Start t1 timer (e.g., every 10 seconds)
        t2.attach(10, ChecktoDeleteID); // Start t2 timer (e.g., every 10 seconds)
...
void confirmAdding(int id) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    String getData = "?confirm_id=" + String(id) + "&device_token=" + String(device_token); // Declare getData
    String Link = URL + getData; // Declare Link
